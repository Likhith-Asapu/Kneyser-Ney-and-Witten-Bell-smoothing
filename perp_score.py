...
trainlist = textlist[:-1000]
testlist = textlist[-1000:]
...
def KN_smoothing(trainset,k,testset):
  ...
  for testline in testset:
    ...
    if(perp_score == 0):
      ...
      perp_score = (1 / perp_score) ** (1/prob_count)
      printlist.append([testline[:-1],"\t",perp_score])
      avg_perplexity += perp_score
      numberofsen += 1
  
  print(avg_perplexity/numberofsen)
...
def WB_smoothing(trainset,k,testset):
  ...
  for testline in testset:
    ...
    if(perp_score == 0):
      ...
      perp_score = (1 / perp_score) ** (1/num_prob)
      printlist.append([testline[:-1],"\t",perp_score])
      avg_perplexity += perp_score
      numberofsen += 1
  print(avg_perplexity/numberofsen)
...
if(sys.argv[2] == 'k'):
  ...
  elif sys.argv[4] == 'train':
    KN_smoothing(tokenisedlist,ORDER,trainlist)
elif(sys.argv[2] == 'w'):
  if sys.argv[4] == 'test':
    WB_smoothing(tokenisedlist,ORDER,testlist)
  elif sys.argv[4] == 'train':
    WB_smoothing(tokenisedlist,ORDER,trainlist)
else:
  print("invalid")
